...
const BRAND_CONFIGS = {
  ...
  skoda: {
    ...
    redirectUri:    'myskoda://redirect/oauth2redirect',
    ...
    userAgent:      'MySkoda-android/4.5.1',
    appVersion:     '4.5.1',
    appName:        'MySkoda',
  },
};
...
class AudiApi {
  ...
  setLogger(fn) { this._logger = fn; }
  _debug(...args) { if (this._logger) this._logger(...args); }
  ...
  async _fetch(method, url, opts = {}, jar) {
    ...
    try {
      ...
      if (err.response) {
        jar.ingest(err.response.headers || {});
        const body = typeof err.response.data === 'string'
          ? err.response.data.slice(0, 300)
          : JSON.stringify(err.response.data || '').slice(0, 300);
        throw new Error(`HTTP ${err.response.status} ${method} ${url} — ${body}`);
      }
      throw err;
    }
  }
  ...
  async login(username, password) {
    this._debug('login: starting auth for brand', this._brand);
  ...
  async _followToCode(jar, resp, depth) {
    ...
    throw new Error(`Auth flow ended without a ${this._cfg.redirectUri} redirect (status ${resp.status})`);
  }
  ...
  async _exchangeCode(code, verifier) {
    ...
    let idkResp;
    try {
      idkResp = await axios.post(IDK_TOKEN_URL,
        qs.stringify({
      ...
      const body = err.response
        ...
        : err.message;
      throw new Error(`IDK token exchange failed (${err.response?.status ?? '?'}): ${body}`);
    }
    ...
    this._debug('exchangeCode: IDK token obtained');
    ...
    try {
      ...
      this._debug('exchangeCode: app token obtained');
    } catch (_) {
    ...
    try {
      ...
      this._debug('exchangeCode: VW MBB token obtained');
    } catch (_) {
  ...
  async refreshTokens() {
    this._debug('refreshTokens: requesting new IDK token');
  ...
  async getVehicles() {
    if (this._brand === 'skoda') {
      // Skňoda: use REST vehicle list endpoint (IDK token is sufficient)
      for (const path of ['', '/user-vehicles']) {
        try {
          const resp = await axios.get(STATUS_BASE + path, {
            headers: {
              'user-agent': this._cfg.userAgent,
  ...
  async getVehicleStatus(vin) {
    this._debug('getVehicleStatus:', vin);
  ...
  async _command(vin, resource, action, body = {}) {
    // Commands require the IDK access token (jtt: "access"); audiToken has empty jtt and is rejected
    this._debug('command:', `${resource}/${action}`, JSON.stringify(body));
    const resp = await axios.post(`${STATUS_BASE}/${vin}/${resource}/${action}`, body, {
      headers: {
        'content-type': 'application/json',
        'accept': '*/*',
        'user-agent': this._cfg.userAgent,
    ...
    if (resp.status >= 400) {
      throw new Error(`Command ${resource}/${action} failed: HTTP ${resp.status} — ${JSON.stringify(resp.data)}`);
    }
  }
